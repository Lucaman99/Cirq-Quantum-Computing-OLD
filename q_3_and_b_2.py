# QUANTUM PERCEPTRON SIMULATION WITH 3 QUBITS AND 2 BIAS
...
moment0 = cirq.Moment([cirq.H.on(q[0]), cirq.H.on(q[1])])
moment1 = cirq.Moment([cirq.H.on(q[2])])

circuit = cirq.Circuit((moment0, moment1))

def tof(f):

    yield cirq.CCX.on(q[0], q[1], work[0])

    for i in range (2, f):
        yield cirq.CCX.on(q[i], work[i-2], work[i-1])
    yield cirq.CNOT.on(work[f-2], target[0])
    counter = f-1
    for i in range (2, f):
        yield cirq.CCX.on(q[counter], work[counter-2], work[counter-1])
        counter = counter - 1
    yield cirq.CCX.on(q[0], q[1], work[0])

circuit.append(tof(3))
...
result = simulator.run(circuit, repetitions=80)
print(result)
