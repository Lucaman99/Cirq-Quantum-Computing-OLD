...
set_edges = [Edge(0, 1), Edge(1, 2), Edge(2, 3)]
...
def apply_C_unitary(length, gamma):
    ...
    #Used for Bernie Sanders demo code
    for j in set_edges:
...
#NEED TO SIMULATE BOTH CIRCUITS!!!
...
#leave commented for Bernie Sanders demo
    ...
    for i in range(0, len(list(dict(final2).keys()))):
        #print(list(dict(final2).keys()))
        hg = entry_number.index(list(dict(final2).keys())[i])
        #print(entry_score)
        entry_score[hg] += i
        #print("Adding"+str(i)+" to "+str(list(dict(final2).keys())[i]))

    for i in range(0, len(list(dict(final).keys()))):
        entry_score[entry_number.index(list(dict(final).keys())[i])] += i
